...
@interface ADLEvaluateWaitView ()<UITableViewDelegate,UITableViewDataSource,ADLEvaluateGoodsCellDelegate>
...
@implementation ADLEvaluateWaitView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        cell.evaluateBtn.layer.borderColor = APP_COLOR.CGColor;
        cell.delegate = self;
    }
...
- (void)didClickEvaluateBtn:(UIButton *)sender {
    ...
    if (self.clickGoEvaluate) {
        self.clickGoEvaluate(self.dataArr[indexPath.row]);
    }
}
...
- (ADLBlankView *)blankView {
    if (_blankView == nil) {
        _blankView = [ADLBlankView blankViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-NAVIGATION_H-VIEW_HEIGHT) imageName:nil prompt:@"暂无待评价商品或服务" backgroundColor:COLOR_F2F2F2];
    }
    return _blankView;
}
