...
@interface ADLRecordDataView ()<UITextFieldDelegate, ADLTextViewDelegate>
@property (nonatomic, strong) NSMutableArray *provinceArr;

@end

@implementation ADLRecordDataView

- (void)awakeFromNib {
    [super awakeFromNib];
    self.briefView.layer.borderWidth = 0.5;
    self.situationView.layer.borderWidth = 0.5;
    self.submitBtn.layer.cornerRadius = CORNER_RADIUS;
    self.briefView.layer.borderColor = COLOR_D3D3D3.CGColor;
    self.situationView.layer.borderColor = COLOR_D3D3D3.CGColor;
    
    CGFloat wid = SCREEN_WIDTH-24;
    ADLTextView *briefTV = [[ADLTextView alloc] initWithFrame:CGRectMake(0, 0, wid, 135) limitLength:200];
    briefTV.font = [UIFont systemFontOfSize:14];
    briefTV.placeholder = @"请输入项目简述";
    briefTV.delegate = self;
    [self.briefView addSubview:briefTV];
    self.briefTV = briefTV;
    
    ADLTextView *situationTV = [[ADLTextView alloc] initWithFrame:CGRectMake(0, 0, wid, 135) limitLength:200];
    situationTV.font = [UIFont systemFontOfSize:14];
    situationTV.placeholder = @"请输入项目竞争情况";
    situationTV.delegate = self;
    [self.situationView addSubview:situationTV];
    self.situationTV = situationTV;
    
    ADLTextView *rangeTV = [[ADLTextView alloc] initWithFrame:CGRectMake(0, 0, wid-140, 100) limitLength:0];
    rangeTV.font = [UIFont systemFontOfSize:14];
    rangeTV.placeholder = @"请输入经营范围";
    rangeTV.delegate = self;
    [self.rangeView addSubview:rangeTV];
    self.rangeTV = rangeTV;
    ...
    UITapGestureRecognizer *areaLabTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(clickSlectAreaLab)];
...
#pragma mark ------ 选择项目所在地 ------
- (void)clickSlectAreaLab {
    [self clickSlectProjectArea];
}

- (IBAction)clickSlectAreaBtn:(UIButton *)sender {
    [self clickSlectProjectArea];
}

- (void)clickSlectProjectArea {
    [self endEditing:YES];
    [ADLAddressPickerView showWithArray:self.provinceArr level:4 finish:^(NSString *address, NSString *addressId) {
        ...
        self.projectAreaId = addressId;
    }];
}

#pragma mark ------ 营业执照类型 ------
- (void)clickLicenseTypeLab {
    [self clickSelectLicenseType];
}
...
- (IBAction)clickDeleteLicenseImageBtn:(UIButton *)sender {
    sender.hidden = YES;
...
- (IBAction)clickOpenDateBtn:(UIButton *)sender {
    [self clickOpenDueDate];
}
...
- (void)clickOpenDueDate {
    ...
    [ADLSelectDateView showWithTitle:@"营业期限" period:YES longterm:YES posterior:NO finish:^(NSString *dateStr) {
        ...
        if ([dateArr.lastObject isEqualToString:@"长期"]) {
            ...
            self.licenseEndTime = dateArr.lastObject;
        }
...
#pragma mark ------ 删除第一张证件图片 ------
- (IBAction)clickDeleteIDImageOneBtn:(UIButton *)sender {
...
#pragma mark ------ 删除第二张证件图片 ------
- (IBAction)clickDeleteIdImageTwoBtn:(UIButton *)sender {
    sender.hidden = YES;
    self.idImage2 = nil;
    self.idImage2Url = nil;
    self.idImgView2.image = [UIImage imageNamed:@"img_upload"];
...
- (IBAction)clickCompanyAddressBtn:(UIButton *)sender {
    [self clickCompanyArea];
}
...
#pragma mark ------ 删除银行开户许可证图片 ------
- (IBAction)clickDeleteBankImageBtn:(UIButton *)sender {
    sender.hidden = YES;
    self.bankImage = nil;
...
- (IBAction)clickSubmitBtn:(UIButton *)sender {
    if (self.nameTF.text.length == 0) {
        [ADLToast showMessage:@"请输入项目名称"];
        return;
    }
    if (self.numTF.text.length == 0) {
        [ADLToast showMessage:@"请输入数量"];
        return;
    }
    if (self.projectAreaId.length == 0) {
        [ADLToast showMessage:@"请选择项目所在地"];
        return;
    }
    if (self.addressTF.text.length == 0) {
        [ADLToast showMessage:@"请输入项目详细地址"];
        return;
    }
    if (self.briefTV.text.length == 0) {
        [ADLToast showMessage:@"请输入项目需求简述"];
        return;
    }
    if ([ADLUtils hasEmoji:self.briefTV.text]) {
        [ADLToast showMessage:@"请不要输入表情和特殊符号"];
        return;
    }
    if (self.situationTV.text.length == 0) {
        [ADLToast showMessage:@"请输入项目竞争情况"];
        return;
    }
    if ([ADLUtils hasEmoji:self.situationTV.text]) {
        [ADLToast showMessage:@"请不要输入表情和特殊符号"];
        return;
    }
    ...
    [params setValue:self.nameTF.text forKey:@"projectName"];
    [params setValue:self.numTF.text forKey:@"num"];
    [params setValue:self.projectAreaId forKey:@"projectAreaId"];
    [params setValue:self.addressTF.text forKey:@"address"];
    [params setValue:self.briefTV.text forKey:@"projectNeedDesc"];
    [params setValue:self.situationTV.text forKey:@"projectCompeteDesc"];
...
- (void)updateInputWithDictionary:(NSDictionary *)dict {
    self.nameTF.text = dict[@"projectName"];
    self.numTF.text = [dict[@"num"] stringValue];
    self.areaLab.textColor = COLOR_333333;
    self.projectAreaId = [dict[@"projectAreaId"] stringValue];
    self.areaLab.text = [ADLUtils queryAddressWithDataArr:self.provinceArr areaId:self.projectAreaId].address;
    
    self.addressTF.text = dict[@"address"];
    self.briefTV.text = dict[@"projectNeedDesc"];
    self.situationTV.text = dict[@"projectCompeteDesc"];
...
- (void)setInputViewUneditable {
    ...
    self.numTF.enabled = NO;
    self.areaLab.userInteractionEnabled = NO;
    self.addressTF.enabled = NO;
    self.briefTV.userInteractionEnabled = NO;
    self.situationTV.userInteractionEnabled = NO;
    ...
    self.nameTF.alpha = 0.5;
    self.numTF.alpha = 0.5;
    self.areaLab.alpha = 0.5;
    self.addressTF.alpha = 0.5;
    self.briefTV.alpha = 0.5;
    self.situationTV.alpha = 0.5;
