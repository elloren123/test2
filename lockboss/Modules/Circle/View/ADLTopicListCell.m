...
@implementation ADLTopicListCell
...
- (void)clickUserIcon:(UITapGestureRecognizer *)tap {
    if (self.delegate && [self.delegate respondsToSelector:@selector(didClickUserIcon:)]) {
        [self.delegate didClickUserIcon:(UIImageView *)tap.view];
    }
}
...
- (IBAction)clickPraiseBtn:(UIButton *)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(didClickPraiseBtn:)]) {
        [self.delegate didClickPraiseBtn:sender];
    }
}
