...
@implementation ADLGoodsEvaluateCell
...
- (void)clickUserIcon {
    if (self.delegate && [self.delegate respondsToSelector:@selector(didClickUserIcon:)]) {
        [self.delegate didClickUserIcon:self.iconView];
    }
}
...
- (IBAction)clickLikeBtn:(UIButton *)sender {
    if (self.delegate && [self.delegate respondsToSelector:@selector(didClickLikeBtn:)]) {
        [self.delegate didClickLikeBtn:sender];
    }
}
...
- (void)updateImageViewImage:(NSArray *)urlArr width:(CGFloat)wid {
    ...
    switch (imgCount) {
        case 0:
            self.imgBgH.constant = 1;
            break;
        case 1: {
            self.imgBgH.constant = 150;
