...
-(void)setView{
    ...
    //选中时显示的图片
    imgview = [[UIView alloc]initWithFrame:CGRectMake(5, 10, self.bounds.size.width- 10, self.bounds.size.height - 10)];
...
//    [self addSubview:imghuoche];
    ...
    //日期
    day_lab = [[UILabel alloc]initWithFrame:CGRectMake(0, 5, self.bounds.size.width, self.bounds.size.width-10)];
    day_lab.textAlignment = NSTextAlignmentCenter;
    day_lab.font = [UIFont systemFontOfSize:14];
    [self addSubview:day_lab];
    
    //农历
    day_title = [[UILabel alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-25, self.bounds.size.width, 13)];
    ...
    switch (calenderDayModel.style) {
            ...
            //将来的日期
        {
            ...
            if (calenderDayModel.holiday) {
                ...
                day_lab.text = [NSString stringWithFormat:@"%lu",(unsigned long)calenderDayModel.day];
                day_lab.textColor = COLOR_E0212A;
            }
            day_title.text = calenderDayModel.Chinese_calendar;
        ...
        case CellDayTypeendTime://被点击的日期
            [self hidden_NO];
            day_lab.text = [NSString stringWithFormat:@"%lu",(unsigned long)calenderDayModel.day];
            day_lab.textColor = [UIColor whiteColor];
            day_title.textColor = [UIColor whiteColor];
